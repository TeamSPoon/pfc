...
mp2pfc(PTA,PTAA):- compound(PTA), PTA=..[t,F|Args],atom(F),mp2pfc_l(Args,ArgsO),PTAA=..[F|ArgsO].

mp2pfc(PA,PAA):- compound(PAA),PAA=..[FF|AABB],!,functor(PAA,FF,A),pfc_to_mp_f(FF,A,F),mp2pfc_l(AABB,AB),PA=..[F|AB].
mp2pfc(PA,PAA):- compound(PA),  PA=..[F|AB],!, functor(PA,F,A),  pfc_to_mp_f(F,A,FF),mp2pfc_l(AB,AABB),PAA=..[FF|AABB].
