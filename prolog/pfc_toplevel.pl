...
:- must(retract(baseKB:'wusing_pfc'(M,CM,SM,pfc_toplevel))),
   show_wdmsg(baseKB:'chusing_pfc'(M,CM,SM,pfc_toplevel)),
   (M==SM -> 
     (SM:use_module(pfc_3.0/pfc_rt),maybe_ensure_abox(SM),nop((M:ain(genlMt(SM,baseKB)))));
     show_wdmsg(baseKB:'lusing_pfc'(M,CM,SM,pfc_toplevel))),   
